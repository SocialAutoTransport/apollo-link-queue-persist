...
var QueuePersistor = (function () {
    function QueuePersistor(options) {
        ...
        this.queue.queueLink.addLinkQueueEventListener("mutation", "enqueue", function (item) {
            console.log('QueuePersistor: mutation enqueued', item);
            _this.persistor.persist();
        });
        this.queue.queueLink.addLinkQueueEventListener("mutation", "dequeue", function (item) {
