...
var Queue = (function () {
    function Queue(options) {
        var queueLink = options.queueLink, _a = options.serialize, serialize = _a === void 0 ? true : _a, client = options.client, beforeRestore = options.beforeRestore, onCompleted = options.onCompleted;
        ...
        if (parsedData != null) {
            parsedData.map(function (graphqlRequest) {
                ...
                if (_this.queueLink.isType(query, 'mutation')) {
                    _this.client.mutate({ mutation: query, variables: variables, context: context }).then(function (response) {
                        if (_this.onCompleted)
                            _this.onCompleted(workingGraphqlRequest, response);
                    });
                }
                else {
                    _this.client.query({ query: query, variables: variables, context: context }).then(function (response) {
                        if (_this.onCompleted)
                            _this.onCompleted(workingGraphqlRequest, response);
                    });
                }
